...
class ProfileController {
  ...
  static Future<ApiResponse<AddressView>> getAddressByMain() async {
    try {

      Uri uri = Uri.http(Config.apiURL, '/api/address/main');
      ...
      if (response.statusCode == 200) {
        ...
        final addressView = AddressView.fromJson(mapResponse);

        addressView.save();

        return ApiResponse.ok(addressView);
      } else {
        Map<String, dynamic> mapResponse = json.decode(response.body);
        ErrorView error = ErrorView.fromJson(mapResponse);
        return ApiResponse.errors(error.messages);
